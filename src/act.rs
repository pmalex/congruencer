use crate::binary_operation::Element;
use itertools::{
...
impl Act {
    pub fn from_cayley_table(table: &[Element], columns: usize) -> Self {
    ...
    pub fn m(&self, class: Element, s: usize) -> Element {
        // Check that the number (class) and the table are compatible.
        debug_assert!(
            ((Element::BITS - class.leading_zeros() - 1) as usize) // Position of the most significant unity.
            <
        ...
        let (result, _) = (0..Element::BITS as usize)
            .fold_while((0 as Element, class), |(accumulator, class), i| {
                if class == 0 {
                    Done((accumulator, class))
                } else if class & 1 != 0 {
                    let result_position = 1 << self.cayley_table[i * self.columns + s];
